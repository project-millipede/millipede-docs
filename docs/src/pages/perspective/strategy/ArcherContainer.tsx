...
import { AnchorPositionType, EntityRelationType, SourceToTargetType } from './types';
...
export class ArcherContainer extends React.Component<Props, State> {
    ...
    return this._getSourceToTargets().map(
      ({ source, target, label, style }: SourceToTargetType) => {
        ...
        const strokeDasharray =
          (style && style.strokeDasharray) || this.props.strokeDasharray;
   ...
   * */
  _generateAllArrowMarkers = (): ReactNode => {
    return this._getSourceToTargets().map(
      ({
        source,
        target,
